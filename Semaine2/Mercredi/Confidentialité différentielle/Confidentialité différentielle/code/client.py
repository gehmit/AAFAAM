...
class MnistClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config=None):
        ...
        # Ajout de bruit gaussien (Confidentialité différentielle)
        epsilon = 1.0
        delta = 1e-5
        noise_scale = 1.0 / epsilon  # bruit simple (à adapter selon les besoins réels)

        noisy_coef = coef + np.random.normal(loc=0.0, scale=noise_scale, size=coef.shape)
        noisy_intercept = intercept + np.random.normal(loc=0.0, scale=noise_scale, size=intercept.shape)
